...
exports.findNeighbors = function(grid, emptyTile){
  ...
  if(grid[emptyTile.row-1,emptyTile.column]){
    neighbors['up'] = new Coordinates(emptyTile.row-1,emptyTile.column);
  }
  if(grid[emptyTile.row+1][emptyTile.column]){
    neighbors['down'] = new Coordinates(emptyTile.row+1,emptyTile.column);
  }
  if(grid[emptyTile.row][emptyTile.column-1]){
    neighbors['left'] = new Coordinates(emptyTile.row,emptyTile.column-1);
  }
  if(grid[emptyTile.row][emptyTile.column+1]){
   neighbors['right'] =  new Coordinates(emptyTile.row,emptyTile.column+1);
  }
...
function Coordinates(row, column){
  ...
  this.getCoordinates = function(){
    return new Array(this.column,this.row);
  }
}
