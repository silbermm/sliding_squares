 ...
 */
exports.Coordinate = function Coordinates(row, column){
  this.row = row;
  this.column = column;

  /**
   * A method that returns the coordinates in 
   * array format with column first
   */
  this.getCoordinates = function(){
    return new Array(this.column,this.ro);
  }
}
...
exports.findEmpty = function(grid){
  ...
  _.each(grid,function(val,idx){
    ...
    if(found > -1) {
      ret = new exports.Coordinates(idx,found);
      return false;
    }
  });
  return ret;
...
exports.findNeighbors = function(grid){
  ...
  if(grid[emptyTile.row-1][emptyTile.column]){
    neighbors['up'] = new exports.Coordinates(emptyTile.row-1,emptyTile.column);
  }
  if(grid[emptyTile.row+1][emptyTile.column]){
    neighbors['down'] = new exports.Coordinates(emptyTile.row+1,emptyTile.column);
  }
  if(grid[emptyTile.row][emptyTile.column-1]){
    neighbors['left'] = new exports.Coordinates(emptyTile.row,emptyTile.column-1);
  }
  if(grid[emptyTile.row][emptyTile.column+1]){
   neighbors['right'] =  new exports.Coordinates(emptyTile.row,emptyTile.column+1);
  }
 ...
 * @param the coordinates (row,col) of the tile to move
 * @return
 */
